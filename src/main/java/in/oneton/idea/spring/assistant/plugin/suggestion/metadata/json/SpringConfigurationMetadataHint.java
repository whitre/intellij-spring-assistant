...
public class SpringConfigurationMetadataHint implements JsonPostProcessable {
    ...
    public String getExpectedPropertyName() {
        return VALUE_REGEX_PATTERN_FOR_MAP.matcher(KEY_REGEX_PATTERN_FOR_MAP.matcher(name).replaceAll("")).replaceAll("");
    }
    ...
    @Override
    public void doOnJsonDeserialization() {
    ...
    public SpringConfigurationMetadataHintValue findHintValueWithName(String pathSegment) {
        ...
        if (value == null) {
            if (providers != null && stream(providers).anyMatch(provider -> provider.getType() == any)) {
                value = SpringConfigurationMetadataHintValue.builder().nameAsObjOrArray(pathSegment).build();
            }
        }
        return value;
    }
